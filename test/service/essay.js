...
describe('test service', next => {
    ...
    describe('test essay', () => {
        ...
        it('should throw an error when add a essay without title or content', async () => {
            await essayService.create({ content: 'test3 without title' }).should.be.rejectedWith(Error);
            await essayService.create({ title: 'test4 without content' }).should.be.rejectedWith(Error);
        })
        ...
        it('should return corrent data when query essay list', async () => {
            for (let i=0; i<5; i++)
                essayService.create({ title: 'fill' + i, content: 'fill' + i });
